...
<body class="horizontal-nav boxed skin-megna card-no-border">
  ...
  if(isset($_POST['valider']))
      {
        $rep= $con ->prepare("select * from utilisateur p WHERE p.login=:login AND p.password=:password AND fonction='admin'");
        $rep->execute(array('login'=>$_POST['login'],'password'=>$_POST['password']));
          ...
          $rep= $con ->prepare("select * from utilisateur p WHERE p.login=:login AND p.password=:password AND fonction='user' ");
              $rep->execute(array('login'=>$_POST['login'],'password'=>$_POST['password']));
          if($data=$rep->fetch())
          {
                    ...
                    <form class="form-horizontal form-material" id="loginform" action="" method="POST">
                        ...
                        <div class="form-group m-b-0">
                            <div class="col-sm-12 text-center">
                                Voulez-vous créer un compte ?<a href="enregistrement.php" class="text-info m-l-5"><b>Cliquez ici</b></a>
                            </div>
                        </div>
                    </form>
                    <form class="form-horizontal" id="recoverform" action="index.html">
                        <div class="form-group ">
                            <div class="col-xs-12">
                                ...
                                <p class="text-muted">Enter votre CNI! </p>
                            </div>
                        </div>
                        <div class="form-group ">
                            <div class="col-xs-12">
